...
def model_save(model, words_index):                     # 模型存檔
    model.save('rnn.h5')                         # creates a HDF5 file 'model.h5'
    with open('rnn_word_index.pickle', 'wb') as handle:
...
def model_predict(model, word_index):
    cnt = 1
    INPUT_SENTENCES = []
    with open('./positive.txt','r+') as f:
        for line in f:         
...
if __name__ == '__main__':
    ...
    model, word_index = model_load('rnn.h5','rnn_word_index.pickle')
